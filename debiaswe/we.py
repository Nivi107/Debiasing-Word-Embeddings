    ...
    def __init__(self, fname):
        ...
        if fname.endswith(".bin"):
            ...
            with open(fname, "r", encoding='utf8') as f:
                # Skip the first line if it's a .vec file (header line with vocab size and dim)
                next(f)
                for line in f:
                    s = line.split()
                    word = s[0]
                    ...
                    # Check if the vector size matches the expected dimension
                    if len(vector_values) != 300:
                        print(f"Skipping line due to dimension mismatch: {line}")
                        continue

                    # Convert the vector values to floats and append to the list
                    try:
                        v = np.array([float(x) for x in vector_values])
...
def doPCA(pairs, embedding, num_components = 10):
    ...
    for a, b in pairs:
        try:
            ...
            print(f"Word {e} not found in embedding vocabulary.")
            continue 
        center = (embedding.v(a) + embedding.v(b))/2
